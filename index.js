    ...
    if(templateId){
        let response = await fetch(`https://api.wolfeocrm.com/v1/setting/2/template/${templateId.formId}`, {
            ...
            headers: {
                ...
                'authorization': 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJMVENCV1M4RkRVTkdTMDNUIiwiaXNzIjoiaHR0cHM6Ly9hcGkuaWxvZ2dlcnouY29tL2FwaS92MS9sb2dpbiIsImlhdCI6MTY3NzgzNTgzNSwiZXhwIjoxNjc3ODM5NDM1LCJuYmYiOjE2Nzc4MzU4MzUsImp0aSI6IkxUQ0JXUzhGRFVOR1MwM1QxNjc3ODM1ODM1In0.8q7oC5ktSe49xWy8RuUKelmRHIcMAdH_FbfXdJ_byqc'
            }
        }).then(async (res) => {
            data = await res.json();
        })
            ...
            if (data.attributes.data[allData[i]].fields[j] == data.attributes.data[allData[i]][data.attributes.data[allData[i]].fields[j]].fieldName) {
                ...
                if (data.attributes.data[allData[i]][data.attributes.data[allData[i]].fields[j]].dataType == "picklist") {
                    ...
                    for (var m = 0; m < radioArray.length; m++) {
                        ...
                        console.log('radioOption', option);
                        var input = document.createElement("input");
